...
def ask_name():
    ...
    for event in pygame.event.get():
        ...
        if keystate == pygame.K_BACKSPACE:
            ...
            break
        elif keystate == pygame.KEYUP:
            current_string.append(int(keystate[i]))
            i =+ 1
            draw_text(screen, current_string[i], 60, width/2, 30)
...
def lose_screen():
    screen.blit(background, background_rect)
    draw_text(screen, "GAME OVER", 100, width / 2, height / 3)
    pygame.display.flip()
